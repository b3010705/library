			...
			<thead>
				...
				<g:each in="${librarianInstanceList}" status="i" var="librarianInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${librarianInstance.id}">${fieldValue(bean: librarianInstance, field: "email")}</g:link></td>
					
						<td>${fieldValue(bean: librarianInstance, field: "name")}</td>
